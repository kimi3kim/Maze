...
//		 makeStartGoal(wall); /* スタートとゴールを作成する   */
//	 スタート地点とゴール地点を決める。
...
// 一度動かした道を表示
	 public void printDungeon() {
		 for (int i = 0; i <wall.length ;i++ ) {
			 for(int j = 0; j<wall.length;j++) {
				 if(x+1 <= mazeSize-1 && i <= x+1 && i >= x-1 &&  j >= y-1 &&  j <= y+1) {
					 if (wall[i][j]==0) {
						 System.out.print("  ");
					 } else if (wall[i][j]== 1) {
						 System.out.print("[]");
...
//	 迷路を描写方法を選択
	 public void challengeLevel(int n) {
		 switch(n) {
//		 レベル初級
		 case 1:
			 printMaze();
...
// 　　　レベル超級
		 ...
		 default:
			 System.out.println("1から４を入力してください。");
			 break;
		 }
	 }
